...
import { getDatabaseSchema } from "./Query.controller.js";
...
export const getDatabaseNaturalDescriptions = async (req, res) => {
  try {
    console.log(
      "🔍 Generating natural language descriptions for user databases..."
    );
    ...
    for (const database of databases) {
      try {
        // Pass both database ID and user ID for ownership validation
        const schema = await getDatabaseSchema(database.id, userId);
        ...
        if (!schema || typeof schema !== "object") {
          databaseDescriptions.push({
            ...
            type: "database",
            naturalDescription:
              "Could not generate schema information for this database",
          ...
          for (const column of columns) {
            if (column.name.endsWith("_id")) {
              ...
              for (const targetTable of tables) {
                const singularTarget = targetTable.endsWith("s")
                  ? targetTable.slice(0, -1)
                  : targetTable;

                if (
                  potentialEntity === targetTable ||
                  potentialEntity === singularTarget
                ) {
...
`;
        ...
        try {
          ...
          console.warn(
            `Failed to parse description for database ${database.id}:`,
            error.message
          );

          naturalDescription = {
            summary:
              "Database schema available but description generation failed",
            tableDescriptions: {},
        ...
        console.error(
          `Error processing database ${database.id}:`,
          error
        );
        ...
        databaseDescriptions.push({
          ...
          naturalDescription: {
            ...
            exampleQueries: [
              "Is this database connected properly?",
            ],
          },
          tableCount: 0,
          tables: [],
    ...
    console.error(
      "❌ Error generating database descriptions:",
      error
    );
